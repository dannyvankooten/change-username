...
function enqueue_assets()
{
    ...
    wp_localize_script('change-username', 'change_username', [
...
function ajax_handler()
{
    ...
    $new_username = trim(strip_tags(wp_unslash($_POST['new_username'])));
    $old_username = trim(strip_tags(wp_unslash($_POST['current_username'])));
...
function change_username($old_username, $new_username)
{
    ...
    // change username
    $wpdb->query($wpdb->prepare("UPDATE $wpdb->users SET user_login = %s WHERE user_login = %s", $new_username, $old_username));
